...
################################################################################
    ...
    def _init_buttons(self):
        ...
        self.buttonFrame.grid(row=0,column=3,rowspan=5,columnspan=2,padx=15,pady=10,
    ...
    def _init_plot(self):
        ...
        self.canvas.get_tk_widget().grid(row=0, column=0, columnspan=3,
                                rowspan=10, sticky=Tk.N+Tk.E+Tk.W+Tk.S, pady=5, padx=5)
        self.canvas.show()
...
################################################################################ 
    ...
    def _load_file(self):
        ...
        if not self._plot_exists:
            self._init_buttons()
            self._init_plot()
            self._center_window(self.master.winfo_screenwidth(),self.master.winfo_screenheight())
    ...
    def _next_file(self,event=None):
        ...
        if ind+1>=len(self._allFiles):
            if os.name == 'posix': print "\033[91mReached end of file list! Wrapping around to beginning...\033[0m"
            else: print "Reached end of file list! Wrapping around to beginning..."
    ...
    def _back_file(self,event=None):
        ...
        if ind-1 == -1:
            if os.name == 'posix': print "\033[91mReached beginning of file list! Wrapping around to end...\033[0m"
            else: print "Reached beginning of file list! Wrapping around to end..."
